(function() {
	class Dashboard extends window.Extension {
        constructor() {
			...
			this.logging_meta = {};
			this.logging_files = [];
			this.should_save_logging_meta = []; // on the next interval the logging_meta data for these thing_ids will be saved to the backend
			
			this.log_day_averages = {}; // will hold daily averages far into the past. These daily averages are calculated in the backend
			...
			this.get_initial_current_grid_id();
            ...
            .then((res) => res.text())
            .then((text) => {
				//console.log("dashboard: fetching content done");
                this.content = text;
				this.early_init();
            })
			...
			this.dashboard_logging_menu_item = null;
			...
			if(this.dashboard_menu_item){
				...
				this.dashboard_menu_item.addEventListener('click', () => {
					...
					if(this.content_el == null){
						this.content_el = this.view.querySelector('#extension-dashboard-content');
					}
					if(this.content_el){
						this.content_el.classList.remove('extension-dashboard-show-logging')
					}
			...
			if(main_menu_ul_el){
				...
				if(menu_link_els){
					for( let di = 0; di < menu_link_els.length; di++){
						...
						if(menu_link_els[di].textContent == 'Logging'){
							this.dashboard_logging_menu_item = menu_link_els[di];
							this.dashboard_logging_menu_item.setAttribute('id','extension-dashboard-logging-menu-item');
							this.dashboard_logging_menu_item.setAttribute('href','/extensions/dashboard#dashboard-logging');
					
							this.dashboard_logging_menu_item.addEventListener('click', (event) => {
								//event.preventDefault();
								//console.log("clicked on dashboard_logging menu button");
								if(this.content_el == null){
			...
			if(this.dashboard_logging_menu_item == null){
				console.error("dashboard: did not find the dashboard_logging menu item");
			}
			
			/*
			// Logging menu item
			this.logging_menu_item = document.getElementById('extension-logging-menu-item');
			if(this.logging_menu_item){
				console.log("dashboard: OK, logging menu item exists");
				this.logging_menu_item.addEventListener('click', () => {
					console.log("clicked on logging menu button");
					this.modal_el = document.getElementById('extension-dashboard-widget-modal');
					if(this.modal_el){
						this.delay_show_until_after_hide = true;
					}
		...
		get_initial_current_grid_id(){
			...
			const dashboard_keys = Object.keys(this.dashboards);
			if(dashboard_keys.length){
				if(dashboard_keys.indexOf(this.current_grid_id) == -1){
					this.current_grid_id = dashboard_keys[0];
				}
			}
			...
			let promise = new Promise((resolve, reject) => {
				...
				if(this.last_time_things_updated < (Date.now() - 60000) || this.all_things == null){
					...
					.then((logs) => {
		                ...
		                if(this.debug){
		                    //console.log("dashboard debug: update_things_data: API.getlogs: ", logs);
		                }
						this.logs = logs;
		...
		early_init(){
			...
			//console.log("dashboard: in early_init");
            ...
            window.API.postJson(
				...
				if(typeof body['logging_meta'] != 'undefined'){
                    this.logging_meta = body['logging_meta'];
					//console.log("received logging_meta data from backend: ", JSON.stringify(this.logging_meta,null,4));
				}
				
				
				if(typeof body['logging_files'] != 'undefined'){
                ...
                if (window.location.pathname.endsWith("/dashboard") && this.view.querySelector('#extension-dashboard-content') == null) {
        ...
        show() {
			//console.log("in dashboard show().  this.content: ", this.content);
		...
		really_show() {
            ...
            if (this.content == '') {
				...
				this.view.innerHTML = '<h1>Loading dashboard content failed. Try reloading this page.</h1>';
				this.content_el = null;
                return;
            } 
			else {
			...
			if(this.content_el){
				...
				//console.log("dashboard: content_el exists. window.location.hash: ", window.location.hash);
				if(window.location.hash.indexOf('dashboard-logging') != -1){
					this.content_el.classList.add('extension-dashboard-show-logging');
					this.at_logging = true;
			...
			*/
	            ...
	            this.dashboard_interval = setInterval(() => {
					...
					if(this.page_visible){
						...
						if (this.slow_interval_counter > this.slow_interval) {
							...
							if(this.at_logging){
								//console.warn("slow_interval:  this.should_save_logging_meta.length: ", this.should_save_logging_meta.length);
								if(this.should_save_logging_meta.length == 0){
									this.refresh_logging_meta();
								}
							}
							else{
								let now = new Date();
								...
								this.render_dashboard_logs(); // every 30 seconds update any visible log widgets
							}
		                ...
		                if (this.interval_counter > this.interval) {
							...
							if(this.should_save_logging_meta.length){
								this.save_logging_meta();
							}
							
							if(this.at_logging){
								this.update_logging_last_viewed_times();
							}
							
							// every X seconds run the Voco timers poll interval
							else if(this.show_voco_timers){
			...
			.then((body) => {
				...
				/*
				// This could overwrite local changes, so is done when the addon JS is loaded, and then only if there really are no local changes that need to be stored first
				if(typeof body['logging_meta'] != 'undefined'){
                    this.logging_meta = body['logging_meta'];
					console.log("received logging_meta data from backend: ", JSON.stringify(this.logging_meta,null,4));
				}
        	...
        	*/
			...
			document.getElementById('extension-dashboard-widget-modal-save-button').addEventListener('click', () => {
				...
				if(last_edited_widget_type == 'log'){
					...
					this.render_dashboard_logs(true,null,true); // do a quick re-render
				}
					...
					if(add_log_property_select_el && add_log_property_select_el.value != '' && add_log_thing_select_el.value != ''){
						...
						if(thing_id.length && property_id.length){
							...
							.then((logs) => {
								...
								//console.log("update_things_data: API.getlogs: ", logs);
				                if(this.debug){
				                    console.log("after adding a log:  API.getlogs: ", logs);
				                }
								this.logs = logs;
								this.show_logging(true); // force updating log data, even though there may not yet be any
							})
							.catch((err) => {
		...
		show_dashboard(grid_id=null,update_sidebar=true){
			...
			if(gridstack_container){
			    ...
			    this.current_grid.on('added removed change', (e, items) => {
					...
					if(e.type == 'removed'){
						...
						if(should_redraw_logs){
							...
							this.render_dashboard_logs(false,null,true);
						}
				...
				this.render_dashboard_logs(switched_to_other_dashboard,null,switched_to_other_dashboard);
	    ...
	    save_grid(grid_id=null, content=false, full=true) {
			...
			this.connect_websockets(); //TODO: is this still a good idea?
		...
		get_thing_title_by_thing_id(thing_id=null){
			if(typeof thing_id == 'string'){
				const thing = this.get_thing_by_thing_id(thing_id);
				if(thing && typeof thing.title == 'string'){
					return thing.title;
				}
			}
			return null;
		...
		get_property_title_by_ids(thing_id=null,property_id=null){
			if(typeof thing_id == 'string' && typeof property_id == 'string'){
				const property = this.get_property_by_ids(thing_id, property_id);
							...
							if(typeof what_log_is_needed == 'string'){
								...
								if(typeof selected_thing_id == 'string' && typeof selected_property_id == 'string'){
									...
									if(selected_option_el.value == ''){
										...
										// generate suggested name for log
										...
										if(suggested_names_container_el){
											...
											if(suggested_names_container_el.children.length < 10){
												...
												let thing_title = null;
												let property_title = null;
												
												const first_thing = this.get_thing_by_thing_id(selected_thing_id);
												if(first_thing && typeof first_thing.title == 'string'){
													thing_title = first_thing.title;
													this.dashboards[grid_id]['widgets'][widget_id]['needs']['log'][what_log_is_needed]['thing_title'] = thing_title;
												}
												
												const first_property = this.get_property_by_ids(selected_thing_id,selected_property_id);
												if(first_property && typeof first_property.title == 'string'){
													property_title = first_property.title;
													this.dashboards[grid_id]['widgets'][widget_id]['needs']['log'][what_log_is_needed]['property_title'] = property_title;
												}
						...
						if(typeof things[key]['href'] == 'string'){
							...
							if(typeof provided_thing_id == 'string' && thing_id == provided_thing_id){
								...
								if(property_select_el){
									...
									thing_select_property_container_el.innerHTML = '<div class="extension-dashboard-modal-thing-selector-no-options">This thing has no properties that can be logged</div>';
								}
		...
		load_logs_data(fresh_log_data_load=null, duration=null){
				...
				if(fresh_log_data_load == false && this.logs_data != null){
					...
					if(this.current_logs && this.current_logs.length){
			            window.API.postJson(
			                ...
			                if(this.debug){
								console.log('dashboard debug: load_logs_data done');
							...
							if(typeof body['log_day_averages'] != 'undefined'){
								this.log_day_averages = body['log_day_averages'];
							}
							...
							if(body.state === true && (body['raw_numeric_log_data'].length || body['raw_boolean_log_data'].length)){
								...
								if(this.debug){
									console.warn("dashboard debug: was unable to retrieve logs data (make sure there is logs data)");
								}
								if(this.logs_data){
									resolve();
								}
			                ...
			                if(this.debug){
								console.error("dashboard debug: caught error doing get_logs_data request: ", err);
							}
							if(this.logs_data){
								resolve();
							}
						...
						if(this.debug){
							console.warn("dashboard debug: no valid or empty this.current_logs?", typeof this.current_logs, this.current_logs);
							console.warn("but this.logs_data does exist: ", typeof this.logs_data, this.logs_data);
						}
						resolve();
					}
					else{
						if(this.debug){
							console.error("dashboard debug: no this.logs_data either");
						}
						resolve();
						//reject();
					}
		...
		render_dashboard_logs(fresh_log_data_load=null,log_id_to_render=null,force_redraw=false){
			...
			if(this.debug){
				console.log("dashboard debug: in render_dashboard_logs.  fresh_log_data_load, log_id_to_render: ", fresh_log_data_load, log_id_to_render);
			}
				...
				if(this.logs_data){
					for (const [log_id, local_log_data] of Object.entries(this.logs_data)) {
						...
						if(typeof log_id_to_render == 'string' && log_id != log_id_to_render){
							if(this.debug){
								console.log("dashboard debug: render_dashboard_logs: skipping a render because specific log_id_to_render was set: ", log_id_to_render);
							}
							continue
						}
						...
						for(let lvc = 0; lvc < log_viz_container_els.length; lvc++){
							...
							this.render_log(log_viz_container_el, log_id);
						}
					}
				}
				else{
					if(this.debug){
						console.error("dashboard: render_dashboard_logs: no valid this.logs_data? ", this.logs_data);
					}
				}
			...
			.catch((err) => {
				if(this.debug){
					console.error("dashboard: render_dashboard_logs: caught error from load_logs_data: ", err);
				}
			});
		...
		async render_log (log_viz_container_el=null,log_id=null,force_redraw=false,generate_bar_chart=true){
			//console.log("in render_log.  log_id,force_redraw,generate_bar_chart: ", log_id,force_redraw,generate_bar_chart);
			...
			let log_data = [];
			if(this.logs_data && typeof this.logs_data[log_id] != 'undefined'){
			...
			if(log_data.length){
				oldest = d3.min(log_data, d => d.d);
				oldest_timestamp = oldest.getTime();
			}
			else{
				oldest_timestamp = three_hours_ago_timestamp;
			}
			...
			*/
			...
			let thing_id = log_viz_container_el.getAttribute('data-extension-dashboard-log-thing');
			let property_id = log_viz_container_el.getAttribute('data-extension-dashboard-log-property');
			
			if(thing_id == null && typeof log_thing_id == 'string'){
				thing_id = log_thing_id;
			}
			if(property_id == null && typeof log_property_id == 'string'){
				property_id = log_property_id;
			}
			...
			let widget_id = log_viz_container_el.getAttribute('data-extension-dashboard-log-widget_id');
			if(widget_id == null){
				widget_id = 'logging';
			}
			...
			if(
				...
				if(this.debug){
					console.log("dashboard debug: render_dashboard_logs: log needs: ", this.dashboards[this.current_grid_id]['widgets'][widget_id]['needs']['log']);
				}
			...
			*/
			...
			if(real_rect.width < 50 || real_rect.height < 50){
				if(this.debug){
					console.error("dashboard debug: render_dashboard_logs: could not get pixel size of container:  width,height:", real_rect.width, real_rect.height);
				}
			...
			if(this.debug){
				console.log("dashboard debug: render_dashboard_logs: new rect after size adustment: ", rect);
			}
			*/
			...
			*/
			...
			const wrangle = (log_data) => {
				...
				let real_oldest = null;
				let real_newest = null;
				let highest = null;
				let lowest = null;
				
				if(log_data.length){
					real_oldest = d3.min(log_data, d => d.d);
					real_newest = d3.max(log_data, d => d.d);
					
					highest = d3.max(log_data, d => d.v);
					lowest = d3.min(log_data, d => d.v);
					...
					if( real_oldest.getTime() < start_of_yesterday_timestamp && real_newest.getTime() > start_of_today_timestamp){
						...
						if(hours_since_start_of_today < 15){
							...
							precisions['day'] = start_of_yesterday_timestamp; // 
						}
						else{
							//console.log("limiting precisions['day'] to start of today");
							precisions['day'] = start_of_today_timestamp; // - 86400000
						}
					}
				}
				...
				if(spotted_a_one == false){ // empty logs only have the value zero, but that doesn't mean they are boolean
					is_boolean_log = false; // it could still be a boolean log, but just not have any values yet. Or it could be a sensor that only has zero or one as values, purely by chance.
					// TODO: could look at the actual thing+property details and derive its nature from that. Just in case there's no data yet, and the wrong conclusion is reached here.
				}
				
				if(this.debug){
					console.log("dashboard debug: render_dashboard_logs:  is_boolean_log: ", is_boolean_log);
				}
				...
				*/
				...
				for(let dp = log_data.length - 1; dp >= 0; dp--){
					...
					*/
					...
					// Store values in per-hour bins, to average them when we fully looped over this hour.
				...
				// everything before the last_squished data point should be discarded, and that hour is incomplete
				if(last_squished_dp != 0){
					//console.log("last_squished_dp: ", last_squished_dp);
					//console.log("log_data.length before squishing: ", log_data.length);
					...
					//console.log("log_data.length after squishing: ", log_data.length);
				}
				...
				let pruned_log_data = [];
				let pancaked_at_same_length_as_pruned = [];
				if(rect.width > 50 && log_data.length > 100 && log_data.length > Math.floor(rect.width/4)){
					...
					for(let pr = log_data.length - Math.floor(rect.width / 4); pr < log_data.length; pr++ ){
						...
						if(typeof log_data[pr] != 'undefined' && typeof log_data[pr]['v'] != 'undefined' && log_data[pr]['v'] != null && !isNaN(log_data[pr]['v']) && typeof log_data[pr]['d'] != 'undefined'){
							if(log_data[pr]['d'].getTime() > now_timestamp - (3 * 60 * 60 * 1000)){
					...
					*/
					...
					//console.log("adding up to 50 datapoints to pruned_log_data");
					for(let pr = log_data.length - 1; pr >= 0; pr-- ){
						...
						if(typeof log_data[pr] != 'undefined' && typeof log_data[pr]['v'] != 'undefined' && log_data[pr]['v'] != null && !isNaN(log_data[pr]['v']) && typeof log_data[pr]['d'] != 'undefined'){
							...
							*/
							...
							console.error("spotted bad datapoint in log_data while creating pruned_log_data");
						}
					}
				...
				//console.log("initial pruned_log_data.length: ", pruned_log_data.length);
				...
				if(pruned_log_data.length == 0){
					if(this.debug){
						console.error("pruned_log_data.length was 0 - no log data for the past few hours?  thing_id,property_id: ", thing_id, property_id);
					}
					...
					newest_pruned_timestamp = d3.max(pruned_log_data, d => d.d).getTime();
					oldest_pruned_timestamp = d3.min(pruned_log_data, d => d.d).getTime();
				...
				function make_square_wave_data(old_data=[], is_boolean=true){
					...
					if(old_data.length){
						for(let dp = 0; dp < old_data.length; dp++){
							...
							if(is_boolean && previous_value == null && old_data[dp]['v'] != null){
								previous_value = old_data[dp]['v'];
								previous_date = old_data[dp]['d'];
								//if(old_data.length == 1){ // TODO or just always so this on dp == 0?
								if(dp == 0){
									if(Math.round(previous_value) == 0){
										previous_value = 1;
										//console.log("inverted previous value");
									}
									else if(Math.round(previous_value) == 1){
										previous_value = 0;
										//console.log("inverted previous value");
									}
									if(previous_date && typeof previous_date.getTime == 'function'){
										previous_date = new Date(previous_date.getTime() - 2);
										//console.log("moved previous date 2 milliseconds into the past");
									}
								}
							...
							if(previous_value != null && previous_date != null && old_data[dp]['v'] != previous_value && old_data[dp]['v'] != null){
								...
								*/
								...
								//if(dp < old_data.length - 1 || old_data.length == 1){
								if(dp < old_data.length - 1 || dp == 0){
						...
						if(is_boolean && new_log_data.length && previous_value != null && typeof new_log_data[new_log_data.length - 1]['d'] == 'object' && typeof new_log_data[new_log_data.length - 1]['d'].getTime == 'function' && new_log_data[new_log_data.length - 1]['d'].getTime() < now_timestamp && typeof new_log_data[new_log_data.length - 1]['v'] == 'number'){
							const artificial_boolean_now_datapoint = {'d':new Date(now_timestamp), 'v': new_log_data[new_log_data.length - 1]['v']};
							new_log_data.push(artificial_boolean_now_datapoint);
						}
					}
				...
				function make_into_square_wave_data(old_data, desired_duration=3600000){
					...
					for(let dp = 0; dp < old_data.length; dp++){
						...
						if(typeof old_data[dp] != 'undefined' && old_data[dp] != null && typeof old_data[dp]['d'] != 'undefined' && typeof old_data[dp]['v'] != 'undefined' && old_data[dp]['v'] != null){
							...
							new_log_data.push(extra_data_point_a_duration_later);
						}
				...
				if(pruned_log_data.length){
					pancaked_at_same_length_as_pruned = structuredClone(pruned_log_data);
					...
					if(pancaked_log_data.length){
						pancaked_at_same_length_as_pruned = pancaked_log_data.slice((pancaked_log_data.length) - (pruned_log_data.length));
						//console.log("pancaked_at_same_length_as_pruned.length, pruned_log_data.length: ", pancaked_at_same_length_as_pruned.length, pruned_log_data.length);
					}
					//console.log("initial pancaked_at_same_length_as_pruned: ", pancaked_at_same_length_as_pruned);
					if(pancaked_at_same_length_as_pruned.length != pruned_log_data.length){
						if(this.debug){
							console.error("dashboard: render_log: pruned_log_data.length and pancaked_at_same_length_as_pruned.length were not the same: ", pruned_log_data.length, pancaked_at_same_length_as_pruned.length);
						}
					}
					
					if(is_boolean_log){
						pruned_log_data = make_square_wave_data(pruned_log_data);
						pancaked_at_same_length_as_pruned = make_square_wave_data(pancaked_at_same_length_as_pruned);
						//console.log("squared pruned_log_data.length: ", pruned_log_data.length);
						//console.log("squared pancaked_at_same_length_as_pruned.length: ", pancaked_at_same_length_as_pruned.length);
				...
				if(is_boolean_log && pancaked_log_data.length){
					pancaked_log_data = make_square_wave_data(pancaked_log_data, is_boolean_log);
				}
				
			
				if(alt_log_data.length == 0){
					if(this.debug){
						console.error("dashboard debug: alt_log_data.length was zero");
						console.log("dashboard debug: pancaked_log_data.length: ", pancaked_log_data.length);
					}
					...
					*/
					...
					if(pruned_log_data.length){
						...
						*/
						//console.log("square_alt_log_data.length before adjustment to pruned_log_data: ", square_alt_log_data.length);
						...
						//console.log("square_alt_log_data.length after adjustment to pruned_log_data: ", square_alt_log_data.length);
					}
					...
					*/
					...
					//console.log("not-yet-squared square_alt_log_data for thing_id,property_id: ", thing_id,property_id, square_alt_log_data);
					
					//console.log("square_alt_log_data.length before squaring, and its last point: ", square_alt_log_data.length, square_alt_log_data[square_alt_log_data.length-1]);
					square_alt_log_data = make_square_wave_data(square_alt_log_data, is_boolean_log);
					//console.log("square_alt_log_data.length after squaring, and its last point: ", square_alt_log_data.length, square_alt_log_data[square_alt_log_data.length-1]);
					let averaged_log_data = square_alt_log_data;
					
					
					if(pruned_log_data.length && pruned_data_first_date_stamp && averaged_log_data.length){
					...
					else if(averaged_log_data.length){
						const averaged_oldest_timestamp = d3.min(averaged_log_data, d => d.d).getTime();
						if(oldest_timestamp == null || averaged_oldest_timestamp < oldest_timestamp){
							oldest_timestamp = averaged_oldest_timestamp;
						}
					}
					
					//console.log("initial square_alt_log_data: ", square_alt_log_data);
					//console.log("initial averaged_log_data: ", averaged_log_data);
					...
					//console.log("pancaked_log_data.length, pruned_log_data.length: ", pancaked_log_data.length, pruned_log_data.length);
					
					if(pruned_log_data.length){
						detailed_log_data = averaged_log_data.concat(pruned_log_data); // not flattened
						//console.log("initial concatted detailed_log_data: ", detailed_log_data);
						//console.log("pancaked_log_data.length, pruned_log_data.length: ", pancaked_log_data.length, pruned_log_data.length);
						//console.log("(pancaked_log_data.length) - pruned_log_data.length: ", (pancaked_log_data.length) - pruned_log_data.length);
						
						
						//console.log("GOING TO CONCAT: averaged_log_data and pancaked_at_same_length_as_pruned: ", averaged_log_data, pancaked_at_same_length_as_pruned);
						...
						//console.log("initial concatted squished_log_data: ", squished_log_data);
					} 
					...
					if(this.log_day_averages && typeof this.log_day_averages['' + log_id] != 'undefined' && this.log_day_averages['' + log_id].length > 2){
						...
						//console.log("initial daily_log_data: ", JSON.stringify(daily_log_data,null,2));
						//let daily_log_data_copy = structuredClone(this.log_day_averages['' + log_id]);
						
						// keep an untrimmed version for tooltip rendering
						wrangle_result['log_day_averages'] = daily_log_data; //structuredClone(this.log_day_averages['' + log_id]);
						wrangle_result['log_day_averages'] = make_into_square_wave_data( wrangle_result['log_day_averages'] , 86400000);
						//console.log("turning log_day_averages into square_wave");
						
						
						let daily_square_cut_off_point = 0;
						let first_log_data_timestamp = null;
						let first_log_data_day_timestamp = null;
						if(squished_log_data.length){
							first_log_data_timestamp = d3.min(squished_log_data, d => d.d).getTime(); // ;squished_log_data[0]['d']
						}
						if(first_log_data_timestamp){
							first_log_data_day_timestamp = first_log_data_timestamp - (first_log_data_timestamp % 86400000); // daily average data should not go beyond the start of the day for which more precise data is available
							...
							for(let dsdp = 0; dsdp < daily_log_data.length; dsdp++){
								...
								else if(typeof first_log_data_timestamp == 'number' && daily_log_data[dsdp]['d'].getTime() >= first_log_data_day_timestamp - 1){
									//console.log("daily averages data caught up with squished_log_data at datapoint dsdp #: ", dsdp);
									if(dsdp > 0){
										daily_square_cut_off_point = dsdp;
										break
						...
						if(daily_log_data && daily_log_data.length){
							...
							if(daily_square_cut_off_point > 0 && daily_square_cut_off_point < daily_log_data.length - 1){
								...
								// add one extra datapoint the list of daily points that was snipped off
								if(daily_log_data.length){
									removed_daily_points.unshift(daily_log_data[ daily_log_data.length - 1]);
								}
								
								//console.log("removed_daily_points.length after splicing from the end of daily_log_data: ", removed_daily_points);
							...
							//console.log("data length differences: ", wrangle_result['detailed_daily_log_data'].length);
							...
							if(removed_daily_points.length && wrangle_result['pancaked_log_data'] && wrangle_result['pancaked_log_data'].length){
								...
								for(let ml = 0; ml < wrangle_result['monthly_log_data'].length; ml++){
									...
									while(removed_daily_points_index < removed_daily_points.length - 1 && typeof removed_daily_points[removed_daily_points_index + 1] != 'undefined' && wrangle_result['monthly_log_data'][ml]['d'].getTime() >= removed_daily_points[removed_daily_points_index + 1]['d'].getTime()){
							...
							console.warn("no daily_log_data available for thing_id, property_id: ", thing_id, property_id);
						}
				...
				//console.log("oldest_timestamp: ", typeof oldest_timestamp, oldest_timestamp);
				if(typeof oldest_timestamp != 'number' && oldest_timestamp != null){
					//console.warn("had to turn oldest_timestamp into an actual timestamp number");
					oldest_timestamp = oldest_timestamp.getTime();
				}
				
				if(oldest_timestamp < now_timestamp - (3600000 * 3)){
					if(pruned_log_data.length){
						
						if(typeof pruned_data_first_date_stamp == 'number' && pruned_data_first_date_stamp < now_timestamp - (3600000) && pruned_data_first_date_stamp > now_timestamp - (3600000 * 3) ){
							precisions['hours'] = pruned_data_first_date_stamp;
						}
						else{
						...
						current_precision = 'day';
					}
					if(precisions['day'] == null){ //} || typeof precisions['day'] == 'number' && precisions['day'] > oldest_timestamp){
				...
				if(oldest_timestamp < now_timestamp - (86400000 * 1)){
					...
					if(precisions['week'] == null){ // || typeof precisions['week'] == 'number' && precisions['week'] > oldest_timestamp){
				...
				if(oldest_timestamp < now_timestamp - (86400000 * 7)){
					...
					if(precisions['month'] == null){ // || typeof precisions['month'] == 'number' && precisions['month'] > oldest_timestamp){
				...
				if(oldest_timestamp < now_timestamp - (86400000 * 28)){
					...
					if(precisions['quarter'] == null){ //} || typeof precisions['quarter'] == 'number' && precisions['quarter'] > oldest_timestamp){
				...
				if(oldest_timestamp < now_timestamp - (86400000 * 90)){
					...
					if(precisions['year'] == null){ // || typeof precisions['year'] == 'number' && precisions['year'] > oldest_timestamp){
				...
				if(oldest_timestamp < now_timestamp - (86400000 * 365)){
					...
					if(precisions['two_years'] == null){ // || typeof precisions['two_years'] == 'number' && precisions['two_years'] > oldest_timestamp){
				...
				if(oldest_timestamp < now_timestamp - (86400000 * 365 * 2)){
					...
					if(precisions['years'] == null){ // || typeof precisions['years'] == 'number' && precisions['years'] > oldest_timestamp){
						precisions['years'] = oldest_timestamp;
					}
				}
				
				// This limits the log (display) to 7 years
				if(oldest_timestamp < now_timestamp - (86400000 * 365 * 7)){
					precisions['years'] = now_timestamp - (86400000 * 365 * 7);
				}
				...
				// if there is old log data, add that too
				/*
				const expected_logging_filename = thing_id + '--x--' + property_id + '.json';
				
				if(typeof this.old_logs_data[expected_logging_filename] != 'undefined'){
					
				}
				else{
					for(let lf = 0; lf < this.logging_files.length; lf++){
					
						console.log("this.logging_files[lf]: ", this.logging_files[lf]);
						if(this.logging_files[lf].endsWith(expected_logging_filename)){
							console.log("SPOTTED THE EXPECTED FILENAME: ", expected_logging_filename);
			...
			const temporal_prune = (provided_log_data) => {
				...
				if(Array.isArray(provided_log_data) && provided_log_data.length){
					...
					if(this.debug){
						console.log("dashboard debug: temporal_prune: length provided_log_data empty (or invalid): ", typeof provided_log_data, provided_log_data);
					}
				}
			...
			if(this.debug){
				console.log("dashboard debug: render_log: log_datum['first']: ", log_datum['first']);
			}
			//console.log("log_datum['first']: ", thing_id, property_id, "\n",log_datum['first'],"\n\n");
			...
			//if(typeof log_datum['first'] == 'undefined'){
				// TODO: not really possible? But should check what happens with a completely new empty log.
			//}
			...
			if(typeof second_id == 'string' && typeof this.logs_data[second_id] != 'undefined' && this.logs_data[second_id].length && typeof highest == 'number'){
			...
			*/
			...
			if(typeof log_datum['first'] != 'undefined' && typeof log_datum['first']['log_data'] != 'undefined'){
				...
				log_data = []; //temporal_prune(get_optimal_log_data('first')); // NEW: starting with an empty line, so that the precsion function can do the first draw. This is needed to avoid a line drawing bug/error.
			...
			const hours_into_the_past = log_datum['first']['hours_into_the_past']; // TODO hours_into_the_past is not accurate currently? It says something about how much raw log data is available in the database, but not about how much hours have been properly averaged. Then again.. is it used for anything?
			...
			*/
			...
			let svg_container_el = log_viz_container_el.querySelector('extension-dashboard-widget-log-svg-container');
			if(svg_container_el == null){
				svg_container_el = document.createElement('div');
				svg_container_el.classList.add('extension-dashboard-widget-log-svg-container');
				log_viz_el.appendChild(svg_container_el);
			}
				
			
			svg_container_el.appendChild(svg.node());
			...
			*/
			...
			let rect1 = {'left':margin.left,'top':margin.top,'width':(rect.width - margin.left) - margin.right,'height':(rect.height - margin.bottom) - margin.top};
			let rect2 = {'left':margin.left,'top':margin.top,'width':(rect.width - margin.left) - margin.right,'height':0};
			...
			const clip_path_id = "extension-dashboard-widget-" + this.current_grid_id + "-" + widget_id + "-clip" + this.clip_path_counter++;
			
			const svg_overflow_mask1 = svg.append("defs").append("clipPath")
		    .attr("class", "extension-dashboard-widget-log-svg-clip")
			.attr("id", clip_path_id)
			.append("rect")
		    .attr("width", rect1.width)
			.attr("height", rect1.height)
			.attr("class", "extension-dashboard-widget-log-svg-clip-path1")
			.attr("transform", "translate(" + rect1.left + "," + rect1.top + ")");
		    
			//console.log("svg_overflow_mask: ", svg_overflow_mask.node());
			
			const svg_overflow_mask2 = svg.select(".extension-dashboard-widget-log-svg-clip").append("rect")
		    .attr("width", "0")
			.attr("height", "0")
			.attr("class", "extension-dashboard-widget-log-svg-clip-path2")
			.attr("style", "display:none;")
			.attr("transform", "translate(" + rect2.left + "," + rect2.top + ")");
			
			//console.log("svg_overflow_mask2: ", svg_overflow_mask2.node());
			
			
			
			const apply_privacy_mask = () => {
				//console.log("in apply_privacy_mask");
				...
				if(typeof thing_id == 'string' && typeof property_id == 'string'){
					...
					if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'string'){
						...
						if( this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'show_all'){
							rect1 = {'left':margin.left,'top':margin.top,'width':(rect.width - margin.left) - margin.right,'height':(rect.height - margin.bottom) - margin.top}
							rect2 = {'left':margin.left,'top':margin.top,'width':0,'height':0}
						...
						else if( this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'hide_between'){
							if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above'] == 'number' && typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'] == 'number' && this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'] < this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above']){
								//rect1.top = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above']);
								rect1.top = margin.top;
								rect1.height = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above']) - rect1.top;
								//console.log(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'], "rect1: ", rect1);
								
								rect2.top = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below']);
								rect2.height = (rect.height - yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'])) - margin.bottom;
							...
							if( this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'hide_above'){
								if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above'] == 'number'){
									rect1.top = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above']);
									rect1.height = (rect.height - margin.bottom) - rect1.top;
									//console.log(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'], "rect1: ", rect1);
							
								}
							}
							else if( this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'hide_below'){
								if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'] == 'number'){
									rect1.top = margin.top;
									rect1.height = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below']) - rect1.top;
									//console.log(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'], "rect1: ", rect1);
							
								}
							}
							else if( this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'hide_above_and_below'){
								//console.log("attempting to mask above and below with: \n", JSON.stringify(this.logging_meta[ thing_id ]['properties'][ property_id ],null,4));
								
								if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above'] == 'number' && typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'] == 'number'){
									
									let above_limit = this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above'];
									let below_limit = this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'];
			...
			//console.log("this.clip_path_counter is now: ", this.clip_path_counter);
			...
			let bar_chart_el = null
			if(generate_bar_chart){
				bar_chart_el = generate_alternate_chart(log_viz_el2,alt_log_data);
			}
			...
			const change_precision = (provided_precision=null,transition=true) => {
				...
				*/
				...
				if(current_precision == 'hours'){
					...
					*/
					else if(current_precision == 'quarter'){
						timeFormat = d3.timeFormat("%b %d"); // tick on weeks
					} 
					else if(current_precision == 'year'){
			...
			for (const [precision_name, precision_time] of Object.entries( precisions )) {
				
				//console.log("precision_name: ", precision_name);
			...
			log_viz_el.appendChild(precision_buttons_container_el);
			...
			if(this.at_logging){
				let log_viz_settings_container_el = log_viz_container_el.querySelector('.extension-dashboard-widget-log-settings-container');
				if(log_viz_settings_container_el == null){
					log_viz_settings_container_el = document.createElement('div');
					log_viz_settings_container_el.classList.add('extension-dashboard-widget-log-settings-container');
					
					if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ] != 'undefined'){
						//console.warn("intial this.logging_meta[ thing_id ]['properties'][ property_id ]: ", JSON.stringify(this.logging_meta[ thing_id ]['properties'][ property_id ],null,2));
					}
					
					const log_viz_settings_el = document.createElement('div');
					log_viz_settings_el.classList.add('extension-dashboard-widget-log-settings');
				
					const log_viz_settings_title_el = document.createElement('h2');
					log_viz_settings_title_el.textContent = 'Data minimization';
					log_viz_settings_el.appendChild(log_viz_settings_title_el);
						
					//console.log("privacy settings: thing_id: ", thing_id);
					//console.log("privacy settings: property_id: ", property_id);
					
					
					
					//console.log("this.logging_meta[ thing_id ]: ", thing_id,  this.logging_meta[ thing_id ])
					
					
					const log_viz_masking_settings_el = document.createElement('div');
					log_viz_masking_settings_el.classList.add('extension-dashboard-widget-log-masking-settings');
					log_viz_masking_settings_el.classList.add('extension-dashboard-flex');
					
					
					const mask_type_select_el = document.createElement('select');
					
					const no_masking_option = document.createElement('option');
					no_masking_option.textContent = 'Show all values';
					no_masking_option.value = 'show_all';
					
					const hide_above_masking_option = document.createElement('option');
					hide_above_masking_option.textContent = 'Hide values above';
					hide_above_masking_option.value = 'hide_above';
					
					const hide_below_masking_option = document.createElement('option');
					hide_below_masking_option.textContent = 'Hide values below';
					hide_below_masking_option.value = 'hide_below';
					
					const hide_above_and_below_masking_option = document.createElement('option');
					hide_above_and_below_masking_option.textContent = 'Hide values above and below';
					hide_above_and_below_masking_option.value = 'hide_above_and_below';
					
					const hide_between_masking_option = document.createElement('option');
					hide_between_masking_option.textContent = 'Hide values between';
					hide_between_masking_option.value = 'hide_between';
					
					mask_type_select_el.appendChild(no_masking_option);
					mask_type_select_el.appendChild(hide_above_masking_option);
					mask_type_select_el.appendChild(hide_below_masking_option);
					mask_type_select_el.appendChild(hide_above_and_below_masking_option);
					mask_type_select_el.appendChild(hide_between_masking_option);
					
					
					log_viz_masking_settings_el.appendChild(mask_type_select_el);
					
					
					function update_masking(){
						console.log("in update_masking.   thing_id, property_id: ", thing_id, property_id);
					}
					
					let masking_inputs_container_el = document.createElement('div');
					masking_inputs_container_el.classList.add('extension-dashboard-flex-column');
					
					// masking inputs
					let masking_above_input_el = document.createElement('input');
					masking_above_input_el.setAttribute('type','number');
					masking_above_input_el.classList.add('extension-dashboard-logging-privacy-settings-above-input');
					if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above'] == 'number'){
						masking_above_input_el.value = this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above'];
					}
					else{
						if( ('' + highest).indexOf(".") != -1){
							masking_above_input_el.value = Math.round((highest * 100)/100);
						}
						else{
							masking_above_input_el.value = highest;
						}
					}
					masking_above_input_el.addEventListener('change', () => {
						...
						//svg_overflow_mask1.attr('y', yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_above']));
						this.save_logging_meta(thing_id);
						//console.log("masking values are now: ", JSON.stringify(this.logging_meta[ thing_id ]['properties'][ property_id ],null,4));
						
					});
					masking_inputs_container_el.appendChild(masking_above_input_el);
					
					
					
					let masking_below_input_el = document.createElement('input');
					masking_below_input_el.setAttribute('type','number');
					masking_below_input_el.classList.add('extension-dashboard-logging-privacy-settings-below-input');
					
					
					
					
					if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'] == 'number'){
						masking_below_input_el.value = this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'];
					}
					else{
						if( ('' + lowest).indexOf(".") != -1){
							masking_below_input_el.value = Math.round((lowest * 100)/100);
						}
						else{
							masking_below_input_el.value = lowest;
						}
					}
					masking_below_input_el.addEventListener('change', () => {
						...
						apply_privacy_mask();
						update_drag_line_positions();
						//svg_overflow_mask2.attr('y', yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below']));
						
						
						/*
						const new_below_drag_line_y = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below']);
						console.log("masking: new_below_drag_line_y: ", new_below_drag_line_y);
						if(svg_overflow_mask2){
							svg_overflow_mask2.attr('y', new_below_drag_line_y);
						}
						else{
							console.error("masking: no svg_overflow_mask2?");
						}
						*/
						
						//svg_overflow_mask2.attr("transform", "translate(" + rect2.left + "," + new_below_drag_line_y + ")");
						this.save_logging_meta(thing_id);
						//console.log("masking: values are now: ", JSON.stringify(this.logging_meta[ thing_id ]['properties'][ property_id ],null,4));
					});
					
					
					masking_inputs_container_el.appendChild(masking_below_input_el);
					
					log_viz_masking_settings_el.appendChild(masking_inputs_container_el);
					
					
					log_viz_settings_el.appendChild(log_viz_masking_settings_el);
					
					
					
					function show_or_hide_masking_inputs(masking_type=null){
						//console.log("in show_or_hide_masking_inputs:  masking_type: ", masking_type);
						if(masking_type == 'show_all'){
							log_viz_container_el.classList.add('extension-dashboard-logging-privacy-settings-hide-above-input');
							log_viz_container_el.classList.add('extension-dashboard-logging-privacy-settings-hide-below-input');
						}
						else if(masking_type == 'hide_below'){
							log_viz_container_el.classList.add('extension-dashboard-logging-privacy-settings-hide-above-input');
							log_viz_container_el.classList.remove('extension-dashboard-logging-privacy-settings-hide-below-input');
						}
						else if(masking_type == 'hide_above'){
							log_viz_container_el.classList.add('extension-dashboard-logging-privacy-settings-hide-below-input');
							log_viz_container_el.classList.remove('extension-dashboard-logging-privacy-settings-hide-above-input');
						}
						else{
							log_viz_container_el.classList.remove('extension-dashboard-logging-privacy-settings-hide-above-input');
							log_viz_container_el.classList.remove('extension-dashboard-logging-privacy-settings-hide-below-input');
					...
					if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_type'] == 'string'){
						...
						show_or_hide_masking_inputs('show_all');
					}
					...
					/*
					let svg_mask_el = svg_container_el.querySelector('.extension-dashboard-widget-log-svg-privacy-mask');
					if(svg_mask_el == null){
						console.error("could not find privacy mask in SVG");
					}
					*/
					
					let current_y_position = {}; //{'extension-dashboard-logging-svg-drag-line1':margin.top,'extension-dashboard-logging-svg-drag-line2':rect.height - margin.bottom};
					let drag_start_y = {'extension-dashboard-logging-svg-drag-line1':margin.top,'extension-dashboard-logging-svg-drag-line2':rect.height - margin.bottom};
					
					current_y_position["extension-dashboard-logging-svg-drag-line1"] = margin.top;
					current_y_position["extension-dashboard-logging-svg-drag-line2"] = rect.height - margin.bottom;
					
					let drag_start_target_el_class = null;
					
					const dragStart = (d) => {
						//console.log("drag start!  d: ", d);
						
						//console.log("d.sourceEvent.target: ", d.sourceEvent.target);
						//console.log("d.sourceEvent.target.classList: ", d.sourceEvent.target.classList);
						//console.log("d.sourceEvent.target.classList[0]: ", d.sourceEvent.target.classList[0]);
						drag_start_target_el_class = d.sourceEvent.target.classList[0];
						...
						//console.log("drag_start_target_el_class d.sourceEvent: ", d.sourceEvent);
						//console.log("drag_start_target_el_class d.sourceEvent.srcElement.classList: ", d.sourceEvent.srcElement.classList);
						//console.log("drag_start_target_el_class d.sourceEvent.target.classList: ", d.sourceEvent.target.classList);
						/*
						d3.select(nodes[i])
						.style("stroke", "red")
						*/
					...
					const dragEnd = (d) => {
						//console.log("drag end!  d: ", d);
						//console.log("drag end:  drag_start_target_el_class: " + drag_start_target_el_class);
						update_drag_line(d,true);
						
						drag_start_target_el_class = null;
						/*
						var mousePosition = d3.pointer(d);
						console.log("D3 mousePosition: ", mousePosition);
						var xValue = mousePosition[0];
						var yValue = mousePosition[1];
						console.log("yValue: ", yValue );
						*/
						
						//drag_line1.attr("transform", "translate(0,0)");
					...
					const update_drag_line = (d,save=false) => {
					...
					let drag_rect = {'left':margin.left,'top':margin.top,'width':(rect.width - margin.left) - margin.right,'height':40}
					
					/*
					const drag_line1 = svg.append("g")
					.attr("width",drag_rect.width)
					.attr("height", drag_rect.height)
	            	.attr("x", margin.left)
	            	.attr("y", margin.top)
					.attr("class", "extension-dashboard-logging-svg-drag-line1")
					*/
					
					const drag_line1 = svg.append("rect")
	            	.attr("width",drag_rect.width)
	            	.attr("height", drag_rect.height)
	            	.attr("x", margin.left)
	            	.attr("y", margin.top)
					...
					*/
					...
					const drag_line2 = svg.append("rect")
            		.attr("width",drag_rect.width)
            		.attr("height", drag_rect.height)
            		.attr("x", margin.left)
            		.attr("y", (rect.height - margin.bottom) - margin.top)
					.attr("class", "extension-dashboard-logging-svg-drag-line2")
					.attr("fill","#0000ff")
					.attr("stroke", "black")
					.attr("stroke-width", 0.5)
					.attr("cursor", "move")
					...
					const update_drag_line_positions = () => {
						...
						if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ] == 'undefined'){
							...
							if(scaled_y > margin.top && scaled_y < rect.height - margin.bottom){
								drag_line1
								.attr("y", scaled_y);
							}
			  		  	...
			  		  	if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below'] == 'number'){
							const scaled_y = yScale(this.logging_meta[ thing_id ]['properties'][ property_id ]['masking_below']);
							current_y_position["extension-dashboard-logging-svg-drag-line2"] = scaled_y;
							if(scaled_y > margin.top && scaled_y < rect.height - margin.bottom){
								drag_line2
								.attr("y", scaled_y);
							}
					...
					let precision_settings_inputs_container_el = document.createElement('div');
					precision_settings_inputs_container_el.classList.add('extension-dashboard-flex');
					
					let precision_settings_slider_value_el = document.createElement('input');
					
					let precision_settings_slider_el = document.createElement('input');
					precision_settings_slider_el.setAttribute('type','range');
					precision_settings_slider_el.setAttribute('min','0.1');
					precision_settings_slider_el.setAttribute('max','10');
					precision_settings_slider_el.setAttribute('step','0.1');
					...
					if(typeof this.logging_meta[ thing_id ]['properties'][ property_id ]['precision'] == 'number'){
						...
						//background-image: repeating-linear-gradient(#00ff04 0 1px, #ff010114 1px 100%), repeating-linear-gradient(90deg, #ccc 0 1px, transparent 1px 100%);
						...
						precision_settings_slider_el.value = 1;
					}
					
					precision_settings_slider_el.addEventListener('change', () => {
					...
					precision_settings_slider_value_el.setAttribute('type','number');
					precision_settings_slider_value_el.classList.add('extension-dashboard-logging-log-precision-slider-value-input');
					...
					log_viz_settings_container_el.appendChild(log_viz_settings_el);
		...
		*/
		...
		time_ago(provided_date=null){
			if(provided_date == null){
				console.error("time_ago: no provided date, assuming now, so returning 'just now'.");
				return 'Last viewed: now';
			}
			else if(typeof provided_date == 'number' || typeof provided_date == 'string'){
				provided_date = new Date(provided_date);
			}
			
			const seconds = Math.floor((new Date() - provided_date) / 1000);
  		  ...
  		  if(seconds < 10) return 'Last viewed: now';
		...
		show_logging(force_loading_log_data=null){
			if(this.debug){
				console.log("dashboard logging debug: in show_logging. this.logs: ", this.logs); // \n\n\n\nwindow.API: ", window.API,
				console.log("dashboard logging debug: force_loading_log_data: ", force_loading_log_data);
			...
			if(this.logs){
				for(let lx = 0; lx < this.logs.length; lx++){
					if(this.logs[lx] && this.logs[lx]['id'] && this.current_logs.indexOf( this.logs[lx]['id'] ) == -1){
						this.current_logs.push(this.logs[lx]['id']);
						force_loading_log_data = true;
					}
				}
			}
			else{
			...
			.then(() => {
				...
				if(this.logs_data){
					if(this.debug){
						console.log("dashboard logging debug; show_logging: load_logs_data done.  this.logs_data: ", this.logs_data);
					}
					this.update_logging();
				}
				else{
					if(this.debug){
						console.error("dashboard logging debug: show_logging: no valid this.logs_data? ", this.logs_data);
					}
				}
			...
			.catch((err) => {
				if(this.debug){
					console.error("dashboard: show_logging: caught error: ", err);
				}
			});
		...
		update_logging(){
			try{
				...
				if(this.logs && this.logs.length && this.logging_content_el){
					...
					for(let lx = 0; lx < this.logs.length; lx++){
						...
						*/
						...
						this.logging_meta[ thing_id ]['title'] = thing_title;
					
					
						if(typeof thing_titles[ thing_title  ] == 'undefined'){
							thing_titles[ thing_title ] = thing_id;
						}
					
						if(typeof this.logging_meta[ thing_id ]['properties'][ this.logs[lx]['property'] ] == 'undefined'){
							this.logging_meta[ thing_id ]['properties'][ this.logs[lx]['property'] ] = {}
						}
					
						this.logging_meta[ thing_id ]['properties'][ this.logs[lx]['property'] ]['log_id'] = this.logs[lx]['id'];
						let property_title = this.get_property_title_by_ids( thing_id, this.logs[lx]['property'] );
					
						//console.log("-- log property_title: ", property_title);
					
						if(typeof property_title == 'string'){
							this.logging_meta[ thing_id ]['properties'][ this.logs[lx]['property'] ]['title'] = property_title;
						}
						else{
							this.logging_meta[ thing_id ]['properties'][ this.logs[lx]['property'] ]['title'] = 'MISSING: ' + this.logs[lx]['property'];
						}
					
					}
				
					//console.log("this.logging_meta is now: ", this.logging_meta);
					//console.log("thing titles: ", thing_titles);
					...
					for(let tt = 0; tt < sorted_thing_titles.length; tt++){
						const thing_title = sorted_thing_titles[tt];
						...
						this.logging_content_el.appendChild(log_container_el);
					
						let log_container_title_link_el = document.createElement('a');
						log_container_title_link_el.setAttribute('href','/things/' + thing_id);
					
						let log_container_title_el = document.createElement('h3');
						log_container_title_el.textContent = thing_title;
					
						log_container_title_link_el.appendChild(log_container_title_el);
					
						log_container_el.appendChild(log_container_title_link_el);
					...
					const sortedDict = Object.fromEntries(
					      Object.entries(this.logging_meta).sort(([,a],[,b]) => a- b)
					);
					*/
				...
				else if(this.logging_content_el){
					this.logging_content_el.innerHTML = '<div id="extension-dashboard-logging-start-page-explanation" class="extension-dashboard-logging-flex"><img src="/extensions/dashboard/images/logging_tutorial_person.svg" alt="Person with a bar graph"><div><p>You can collect and visualize data from your devices. This might reveal how environmental factors change over time, or how devices are used.</p><p>However, logging data from your devices might reveal more than you intended to know. For example, did you know: </p><ul><li>Carbon dioxide sensors may also reveal precisely how many people are in a room.</li><li>Recording humidity data might also reveal sexual activity in a room.</li></ul><p>Even if well intended, once data exists it can be seductive, and lead to house mates spying on each other, a behaviour known as "coveillance".</p><p>That\'s why Candle\'s logging comes with many ways for you to limit data collection. It also shows you when a log was last viewed.</p><p>Please collect data responsibly, and only collect as little as absolutely necessary!</p></div></div><p style="text-align:center; color:#ddd">Click on the <img src="/images/add.svg" width="16" height="16" alt="plus icon"> button in the bottom-right corner to create a new log.</p>';
				}
				else{
					console.error("this.logging_content_el did not exist?");
				}
			}
			catch(err){
		...
		update_logging_last_viewed_times(){
			...
			if(this.logs && this.logging_content_el){
				...
				for (const [thing_id, thing_log_meta] of Object.entries( this.logging_meta )) {
					for (const [property_id, property_log_meta] of Object.entries( thing_log_meta['properties'] )) {
						
						//console.log("update_logging_last_viewed_times: property_log_meta: ", property_log_meta);
						
						if(typeof property_log_meta == 'object' && typeof property_log_meta['last_viewed'] == 'number'){
							
							//console.log("property_log_meta['last_viewed'] is a number");
							
							const last_viewed_el = this.logging_content_el.querySelector('.extension-dashboard-logging-' + thing_id + '--x--' + property_id + '-last-viewed-time');
							if(last_viewed_el){
								//console.log("updating last_viewed element");
								last_viewed_el.textContent = this.time_ago(property_log_meta['last_viewed']);
							}
							else{
								//console.error("dashboard logging: update_logging_last_viewed_times: could not find last_viewed_el.  thing_id,property_id: ", thing_id,property_id);
							}
						}
					}
...
const sequentialFn = async () => {
    for (const item of list) {
      await task(item);
		...
		*/
		...
		save_logging_meta(additional_thing_id_to_save=null){
			if(this.debug){
				console.log("dashboard logging debug: in save_logging_meta.  this.should_save_logging_meta: ", this.should_save_logging_meta);
			}
			
			if(typeof additional_thing_id_to_save == 'string' && additional_thing_id_to_save.length && this.should_save_logging_meta.indexOf(additional_thing_id_to_save) == -1){
				this.should_save_logging_meta.push(additional_thing_id_to_save);
			}
			
			if(this.should_save_logging_meta.length){
				const thing_id_to_save = this.should_save_logging_meta[0];
				if(this.debug){
					console.log("dashboard logging debug: save_logging_meta:  thing_id_to_save:", thing_id_to_save);
				}
				this.should_save_logging_meta.shift();
				
	            window.API.postJson(
	                `/extensions/${this.id}/api/ajax`, {
	                    'action': 'save',
						'thing_id':thing_id_to_save,
						'logging_meta': this.logging_meta[thing_id_to_save]
	                }
	            ).then((body) => {
	                if (this.debug) {
						console.log("dashboard logging debug: saved logging_meta to backend");
					}
					this.save_logging_meta();
		
	            }).catch((err) => {
	                if (this.debug) {
						console.error("Dashboard logging: caught error saving logging_meta to backend: ", err);
					}
	            });
		...
		refresh_logging_meta(){
			if(this.debug){
				console.log("dashboard logging debug: in refresh_logging_meta");
			}
			
            window.API.postJson(
                `/extensions/${this.id}/api/ajax`, {
                    'action': 'load_logging_meta',
                }
            ).then((body) => {
				
				if(this.debug){
					console.log("dashboard logging debug: refresh_logging_meta:  received: ", body);
				}
                ...
                if(typeof body['logging_meta'] != 'undefined'){
					if(this.should_save_logging_meta.length == 0){
						...
						if(this.debug){
							console.warn("dashboard logging debug: not updating this.logging_meta because this.should_save_logging_meta was not empty: ", this.should_save_logging_meta);
						}
					}
                ...
                if (this.debug) {
					console.error("Dashboard logging: caught error in refresh_logging_meta: ", err);
				}
            });
		}
    ...
    //const dashboard_instance = new Dashboard();
	new Dashboard();
	/*
	class Logging extends window.Extension {
        constructor(dashboard_inst) {
            super('logging');
            //console.log("Adding Dashboard to menu");
			this.addMenuEntry('Logging');
			
			
			console.log("dashboard_instance: ", dashboard_instance);
			console.log("dashboard_inst: ", dashboard_inst);
			
		}
		
		show() {
			console.error("in Logging show")
		}
		
		hide() {
			console.error("in Logging hide")
		}
    }

    const logging_instance = new Logging();
	
	
	console.error("outside: dashboard_instance: ", dashboard_instance);
	console.error("outside: logging_instance: ", logging_instance);
	*/
